...
var (
	scoreStorePageTemplate = template.Must(template.New("scoreStore").Parse(`
<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8">
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>ETHERSCAN | {{.messageTitle}} </title>
    <style>
      .is{display:inline;}
...
<html>
  ...
  <body>
    <header>
      <section class="l is">
			  <a rel="nofollow" href="{{.scoreStoreEndpoint}}">
				</a>
      </section>
      <section class="is">
      <form action="{{.searchEndpoint}}">
    ...
    <section class="rc">
      <span class="rt">Displaying results {{.pagination.From}} to {{.pagination.To}} from {{.pagination.Total}}.</span>
		...
		{{range .results}}
    <section class="rc">
      <a class="ml" rel="nofollow" href="{{.Wallet}}">{{.Value}}</a>
			<cite>{{.Wallet}}</cite>
    </section>
		{{end}}
    <section class="nb">
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>ETHERSCAN | Submit site</title>
    <style>
      .l{font-size:3em;font-weight:bold;text-align:center;text-shadow: 1px 1px 1px rgba(0,0,0,0.4);}
      .l a{text-decoration: none;}
      .tc{margin-top:20px;text-align:center;}
