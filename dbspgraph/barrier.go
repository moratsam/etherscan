...
func newMasterStepBarrier(ctx context.Context, numWorkers int) *masterStepBarrier {
	...
	// For every step type make a wait channel and a notify channel.
...
func newWorkerStepBarrier(ctx context.Context, stream *remoteMasterStream) *workerStepBarrier {
	// For each step type, create a wait channel.
	waitCh := make(map[proto.Step_Type]chan *proto.Step)
	for stepType := range proto.Step_Type_name {
		if proto.Step_Type(stepType) == proto.Step_INVALID {
			continue
...
// When the worker wants to enter the barrier for a particular step, it creates a new Step
// message with the local state that it wants to share with the master, sends it
// and invokes the blocking Wait method.
...
// Step message response that was passed to Notify by the master.
