...
func (r fifo) Run(ctx context.Context, params StageParams) {
	for {
		...
		case payloadIn, ok := <-params.Input():
			...
			// If the processor did not output a payload for the next stage,
			// mark the payloadIn as processed and continue
			if payloadOut == nil {
				payloadIn.MarkAsProcessed()
				continue
			}

			// Blocking-output the processed data
			select {
...
// to process incoming payloads in parallel and emit their outputs to the next stage.
...
// DynamicWorkerPool returns a StageRunner that maintains a dynamic worker pool that
// can scale up to maxWorkers for processing incoming inputs in parallel
...
stop:
	for {
		...
		case payloadIn, ok := <-params.Input():
			...
			// Wait for a new token from the pool.
			select {
			case <- p.tokenPool:
			case <-ctx.Done():
				break stop
...
// to all specified processors and emits their inputs to the next stage.
...
func (b *broadcast) Run(ctx context.Context, params StageParams) {
	...
	// Start each FIFO in a go-routine. Each FIFO gets its own dedicated input channel
	// and the shared output channel passed to Run.
	...
	for {
		// Read incoming payloads and broadcast them to every FIFO.
		select {
		case <- ctx.Done():
			break done
		case payload, ok := <-params.Input():
			...
			for i:=len(b.fifos)-1; i>=0; i-- {
				// As each FIFO might modify the payload, to avoid data races we need to 
				// make a copy of the payload for all FIFOs except the first.
				...
				case <- ctx.Done():
					break done
				case inCh[i] <- fifoPayload: // payload sent to i-th FIFO.
				}
			}
		}
	}
