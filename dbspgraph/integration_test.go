...
func (j *jobRunner) setupGraph(c *gc.C) {
	graph, err := bspgraph.NewGraph(bspgraph.GraphConfig{
		ComputeFn: func(g *bspgraph.Graph, v *bspgraph.Vertex, msgIt message.Iterator) error {
			...
			// At step 0, each vertex will send a message to the vertex with address 
			// "0000000000000000000000000000000000000000".
			if g.Superstep() == 0 {
				if j.sendBogusMessage {
					_ = g.SendMessage("f000000000000000f00000000000000000000000", graphMessage("message to unknown destination"))
				}
				j.logger.Debugf("[STEP %d] sending message from %v", g.Superstep(), v.ID())
				return g.SendMessage("0000000000000000000000000000000000000000", graphMessage(fmt.Sprintf("hello from %s", v.ID())))
			}
			return nil
		},
	})
...
func (j *jobRunner) StartJob(jobDetails job.Details, execFactory bspgraph.ExecutorFactory) (*bspgraph.Executor, error) {
	...
	if !j.isMaster {
		j.graph.AddVertex(jobDetails.PartitionFromAddr, nil)
	}
...
func (s *serializer) Serialize(v interface{}) (*types.Any, error) {
	switch val := v.(type) {
	case int:
...
func (s *serializer) Unserialize(v *types.Any) (interface{}, error) {
	switch v.TypeUrl {
	case "int":
