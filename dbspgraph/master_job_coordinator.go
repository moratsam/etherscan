...
func newMasterJobCoordinator(ctx context.Context, cfg masterJobCoordinatorConfig) (*masterJobCoordinator, error) {
	partRange, err := partition.NewRange(cfg.jobDetails.PartitionFromAddr, cfg.jobDetails.PartitionToAddr, len(cfg.workers))
	if err != nil {
		return nil, err
	}
...
// publishJobDetails figures out the Address range assignment for a remote worker
...
func (c *masterJobCoordinator) publishJobDetails(w *remoteWorkerStream, assignedPartition int) error {
	partitionFromAddr, partitionToAddr, err := c.partRange.PartitionExtents(assignedPartition)
	if err != nil {
		return xerrors.Errorf("unable to calculate partition assignment: %w", err)
	}

	ts, err := types.TimestampProto(c.cfg.jobDetails.CreatedAt)
	if err != nil {
	...
	c.sendToWorker(w, &proto.MasterPayload{
		Payload: &proto.MasterPayload_JobDetails{
			JobDetails: &proto.JobDetails{
				...
				CreatedAt:         ts,
				PartitionFromAddr: partitionFromAddr[:],
				PartitionToAddr:   partitionToAddr[:],
			},
		},
	})
	return nil
...
func (c *masterJobCoordinator) relayMessageToWorker(srcWorkerIndex int, relayMsg *proto.RelayMessage) {
	...
	dstAddress := relayMsg.Destination

	partIndex, err := c.partRange.PartitionForAddr(dstAddress)
	if err != nil {
